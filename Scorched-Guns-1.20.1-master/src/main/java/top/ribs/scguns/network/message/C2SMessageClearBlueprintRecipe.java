        ...
        context.execute(() -> {
            ...
            if (player != null) {
                ...
                if (blueprint.getItem() instanceof BlueprintItem) {
                    if (ItemStackNbtHelper.getTag(blueprint) != null && ItemStackNbtHelper.getTag(blueprint).contains("ActiveRecipe")) {
                        ItemStackNbtHelper.getTag(blueprint).remove("ActiveRecipe");

                        if (ItemStackNbtHelper.getTag(blueprint).isEmpty()) {
                            ItemStackNbtHelper.removeTag(blueprint);
                        }
                    }
                }
            }
