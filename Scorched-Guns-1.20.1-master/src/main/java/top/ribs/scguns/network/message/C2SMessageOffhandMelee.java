...
public class C2SMessageOffhandMelee extends PlayMessage<C2SMessageOffhandMelee> {
    ...
    private void performOffhandMeleeAttack(ServerPlayer player, Entity target, ItemStack weapon, float attackStrength) {
        ...
        if (damaged) {
            ...
            if (weapon.isDamageableItem()) {
                weapon.hurtAndBreak(1, player, EquipmentSlot.OFFHAND);
            }
    ...
    private float getWeaponAttackSpeed(ItemStack weapon) {
        final float[] attackSpeed = {4.0F};
        weapon.getAttributeModifiers().forEach(net.minecraft.world.entity.EquipmentSlot.MAINHAND, (attribute, modifier) -> {
            if (attribute.is(net.minecraft.world.entity.ai.attributes.Attributes.ATTACK_SPEED)) {
                attackSpeed[0] += (float) modifier.amount();
            }
        });

        return Math.max(0.1F, attackSpeed[0]);
    }
