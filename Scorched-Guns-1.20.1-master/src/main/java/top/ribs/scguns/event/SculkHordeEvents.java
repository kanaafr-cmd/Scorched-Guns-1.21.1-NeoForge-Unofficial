...
public class SculkHordeEvents {
    ...
    public static void onProjectileHit(GunProjectileHitEvent event) {
        ...
        if (result.getType() == HitResult.Type.ENTITY) {
            ...
            if (entity instanceof LivingEntity livingEntity) {
                Holder<MobEffect> purity = getPurityEffect();
                if (purity != null) {
                    livingEntity.addEffect(new MobEffectInstance(purity, 20 * 10));
                }
            }
    ...
    private static Holder<MobEffect> getPurityEffect() {
        try {
            Object purity = Class.forName("com.github.sculkhorde.core.ModMobEffects").getField("PURITY").get(null);
            Object value = purity.getClass().getMethod("get").invoke(purity);
