...
public class RenderUtil
{
    ...
    public static void renderMaceratorWheel(BakedModel model, PoseStack poseStack, MultiBufferSource buffer, int light, int overlay) {
        if (model == null) {
 ...
 public static void renderPolarGeneratorWheel(BakedModel model, PoseStack poseStack, MultiBufferSource buffer, int light, int overlay) {
    ...
    private static void renderQuads(PoseStack poseStack, VertexConsumer buffer, List<BakedQuad> quads, ItemStack stack, ItemStack parent, int light, int overlay)
    {
        ...
        for(BakedQuad quad : quads)
        {
            ...
            buffer.putBulkData(entry, quad, red, green, blue, 1.0F, light, overlay); //TODO check if right
        }
    }
    ...
    public static void renderModel(BakedModel model, PoseStack matrixStack, MultiBufferSource buffer, int combinedLight, int combinedOverlay) {
        RenderUtil.renderModel(model, ItemDisplayContext.NONE, ItemStack.EMPTY, matrixStack, buffer, combinedLight, combinedOverlay);
    }
}
