    ...
    public void slotsChanged(Container container) {
        containerAccess.execute((level, pos) -> {
            if (!level.isClientSide) {
                ...
                Optional<GunBenchRecipe> recipe = level.getRecipeManager().getRecipeFor(GunBenchRecipe.Type.INSTANCE, ScGunsRecipeInput.of(craftingContainer), level)
                        .map(holder -> holder.value());
    ...
    private void consumeIngredients() {
        containerAccess.execute((level, pos) -> {
            if (!level.isClientSide) {
                ...
                Optional<GunBenchRecipe> recipeOptional = level.getRecipeManager().getRecipeFor(GunBenchRecipe.Type.INSTANCE, ScGunsRecipeInput.of(craftingContainer), level)
                        .map(holder -> holder.value());
