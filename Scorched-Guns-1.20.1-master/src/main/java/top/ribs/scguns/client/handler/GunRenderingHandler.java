...
public class GunRenderingHandler {
        ...
        if (Config.CLIENT.display.cooldownIndicator.get()) {
            ...
            if (!gun.getGeneral().isAuto()) {
                ...
                if (coolDown > 0.0F) {
                    ...
                    int progress = Mth.clamp((int) Math.ceil((coolDown + 0.05) * 17.0F) - 1, 0, 16);
                    guiGraphics.fill(j, i, j + 16, i + 4, 0xCC202020);
                    guiGraphics.fill(j, i, j + progress, i + 4, 0xFFE6E6E6);
    ...
    private float getEquipProgress(float partialTicks) {
        if (this.equippedProgressMainHandField == null) {
            this.equippedProgressMainHandField = findItemInHandRendererField("mainHandHeight");
        }
        if (this.prevEquippedProgressMainHandField == null) {
            this.prevEquippedProgressMainHandField = findItemInHandRendererField("oMainHandHeight");
        }
        if(this.equippedProgressMainHandField == null || this.prevEquippedProgressMainHandField == null) {
            return 0.0F;
        }
    ...
    private Field findItemInHandRendererField(String name) {
        try {
            return ObfuscationReflectionHelper.findField(ItemInHandRenderer.class, name);
