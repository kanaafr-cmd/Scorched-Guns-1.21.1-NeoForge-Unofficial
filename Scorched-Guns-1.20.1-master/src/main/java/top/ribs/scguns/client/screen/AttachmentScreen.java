...
public class AttachmentScreen extends AbstractContainerScreen<AttachmentContainer>
{
    ...
    protected void renderLabels(GuiGraphics pGuiGraphics, int mouseX, int mouseY) {
        ...
        pGuiGraphics.pose().last().pose().mul((new Matrix4f()).scaling(1.0F, -1.0F, 1.0F));
        pGuiGraphics.pose().scale(90.0F, 90.0F, 90.0F);
        org.joml.Matrix4fStack modelStack = RenderSystem.getModelViewStack();
    ...
    private void openConfigScreen()
    {
        ...
        {
            Screen screen = IConfigScreenFactory.getForMod(container.getModInfo()).map(factory -> factory.createScreen(container, this)).orElse(null);
            if(screen != null)
            {
