...
public final class MetaLoader implements IDataLoader<MetaLoader.ItemResource>
{
    ...
    private final Object2ObjectMap<Item, DataObject> itemToData = Util.make(new Object2ObjectOpenHashMap<>(), map -> map.defaultReturnValue(DataObject.EMPTY));
    ...
    public List<ItemResource> getResourceSuppliers()
    {
        ...
        BuiltInRegistries.ITEM.stream().toList().stream().filter(item -> item instanceof IMeta).forEach(item ->
        {
