...
//        }
        ...
        if (stack.getItem() instanceof AnimatedGunItem && transformType == ItemDisplayContext.FIRST_PERSON_RIGHT_HAND) {
            ...
            this.applyBobbingTransforms(poseStack, Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(true));
            ...
            this.applySwayTransforms(poseStack, modifiedGun, stack, localPlayer, translateX, translateY, translateZ, Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(true));
            if (ShootingHandler.get().isShooting() && !GunModifierHelper.isSilencedFire(stack)) {
                this.renderMuzzleFlash(Minecraft.getInstance().player, poseStack, bufferSource, stack, ItemDisplayContext.FIRST_PERSON_RIGHT_HAND, Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(true));
            }
        }

        int blockLight = player.isOnFire() ? 15 : player.level().getBrightness(LightLayer.BLOCK, BlockPos.containing(player.getEyePosition(Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(true))));
        if (ShootingHandler.get().isShooting() && !GunModifierHelper.isSilencedFire(stack)) {
        ...
        if (transformType == ItemDisplayContext.GUI) {
            ...
            packedLight = LightTexture.pack(blockLight, player.level().getBrightness(LightLayer.SKY, BlockPos.containing(player.getEyePosition(Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(true)))));
        }
...
//        }
        ...
        switch (bone.getName()) {
            ...
            case "sliding_magazine" -> {
                CompoundTag tag = ItemStackNbtHelper.getOrCreateTag(this.currentRenderStack);
