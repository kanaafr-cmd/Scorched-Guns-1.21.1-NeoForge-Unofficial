...
public class MerchantTradeConfig {
    ...
    private static final ResourceLocation CONFIG_LOCATION = ResourceLocation.fromNamespaceAndPath("scguns", "entity/merchant_trades.json");
    ...
    public static class TradeData {
        ...
        public MerchantOffer toMerchantOffer() {
            if (buyItem2.isEmpty()) {
                return new MerchantOffer(toItemCost(buyItem), sellItem, maxUses, xpReward, priceMultiplier);
            } else {
                return new MerchantOffer(toItemCost(buyItem), Optional.of(toItemCost(buyItem2)), sellItem, maxUses, xpReward, priceMultiplier);
            }
        }

        private static ItemCost toItemCost(ItemStack stack) {
            return new ItemCost(stack.getItemHolder(), stack.getCount(), net.minecraft.core.component.DataComponentPredicate.allOf(stack.getComponents()), stack);
        }
    }
