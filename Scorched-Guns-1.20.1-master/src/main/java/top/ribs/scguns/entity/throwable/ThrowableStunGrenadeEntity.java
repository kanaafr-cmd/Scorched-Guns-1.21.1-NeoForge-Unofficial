...
import top.ribs.scguns.Config.EffectCriteria;
...
public class ThrowableStunGrenadeEntity extends ThrowableGrenadeEntity
{
    ...
    {
        if(Config.COMMON.stunGrenades.blind.blindMobs.get() && event.getOriginalAboutToBeSetTarget() != null && event.getEntity() instanceof Mob && event.getEntity().hasEffect(ModEffects.holder(ModEffects.BLINDED)))
        {
    ...
    public void onDeath()
    {
        ...
        this.level().playSound(null, this.getX(), y, this.getZ(), ModSounds.ENTITY_STUN_GRENADE_EXPLOSION.get(), SoundSource.BLOCKS, 2, (1 + (level().random.nextFloat() - level().random.nextFloat()) * 0.2F) * 0.7F);
        ...
        for(LivingEntity entity : this.level().getEntitiesOfClass(LivingEntity.class, new AABB(minX, minY, minZ, maxX, maxY, maxZ)))
        {
            ...
            if(this.calculateAndApplyEffect(ModEffects.holder(ModEffects.DEAFENED), Config.COMMON.stunGrenades.deafen.criteria, entity, grenade, eyes, distance, angle) && Config.COMMON.stunGrenades.deafen.panicMobs.get())
            {
                entity.setLastHurtByMob(entity);
            }
            if(this.calculateAndApplyEffect(ModEffects.holder(ModEffects.BLINDED), Config.COMMON.stunGrenades.blind.criteria, entity, grenade, eyes, distance, angle) && Config.COMMON.stunGrenades.blind.blindMobs.get() && entity instanceof Mob)
            {
    ...
    private boolean calculateAndApplyEffect(Holder<MobEffect> effect, EffectCriteria criteria, LivingEntity entity, Vec3 grenade, Vec3 eyes, double distance, double angle)
    {
        ...
        if(distance <= criteria.radius.get() && angleMax > 0 && angle <= angleMax)
        {
            ...
            if(effect.value() != ModEffects.BLINDED.get() || !Config.COMMON.stunGrenades.blind.criteria.raytraceOpaqueBlocks.get() || rayTraceOpaqueBlocks(this.level(), eyes, grenade, false, false, false) == null)
            {
        ...
        if(!Double.isNaN(start.x) && !Double.isNaN(start.y) && !Double.isNaN(start.z))
        {
            if(!Double.isNaN(end.x) && !Double.isNaN(end.y) && !Double.isNaN(end.z))
            {
                ...
                while(limit-- >= 0)
                {
                    ...
                    // Added light opacity check
                    if(state.getLightBlock(world, pos) != 0 && (!ignoreBlockWithoutBoundingBox || state.is(Blocks.NETHER_PORTAL)  || state.is(Blocks.END_PORTAL)|| state.getCollisionShape(world, pos) != Shapes.empty()))
                    {
