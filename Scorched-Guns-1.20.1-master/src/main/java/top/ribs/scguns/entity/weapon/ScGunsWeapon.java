...
public class ScGunsWeapon implements IWeapon {
    ...
    public SoundEvent getShootSound() {
        if (fireSound == null)
            fireSound = BuiltInRegistries.SOUND_EVENT.get(
    ...
    public SoundEvent getLoadSound() {
        if (loadSound == null)
            loadSound = BuiltInRegistries.SOUND_EVENT.get(gun.getSounds().getReload());
        return loadSound;
    }
    ...
    public void performRangedAttackIWeapon(Mob shooter, double x, double y, double z, float projectileSpeed) {
        ...
        if (projectileProps.isVisible()) {
            ...
            PacketHandler.getPlayChannel().sendToNearbyPlayers(
                    () -> LevelLocation.create((net.minecraft.server.level.ServerLevel) level, radius, y1, z1, r),
                    messageBulletTrail
            );
        }
    }
