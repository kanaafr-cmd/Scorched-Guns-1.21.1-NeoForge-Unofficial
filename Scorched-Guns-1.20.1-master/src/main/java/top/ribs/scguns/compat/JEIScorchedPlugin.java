...
public class JEIScorchedPlugin implements IModPlugin {
    ...
    public void registerRecipes(IRecipeRegistration registration) {
        ...
        List<GunBenchRecipe> gunBenchRecipes = recipeManager.getAllRecipesFor(GunBenchRecipe.Type.INSTANCE).stream().map(holder -> holder.value()).toList();
        List<MaceratorRecipe> maceratorRecipes = recipeManager.getAllRecipesFor(MaceratorRecipe.Type.INSTANCE).stream().map(holder -> holder.value()).toList();
        List<PoweredMaceratorRecipe> poweredMaceratorRecipes = recipeManager.getAllRecipesFor(PoweredMaceratorRecipe.Type.INSTANCE).stream().map(holder -> holder.value()).toList();
        List<MechanicalPressRecipe> mechanicalPressRecipes = recipeManager.getAllRecipesFor(MechanicalPressRecipe.Type.INSTANCE).stream().map(holder -> holder.value()).toList();

        List<PoweredMechanicalPressRecipe> poweredMechanicalPressRecipes = recipeManager.getAllRecipesFor(PoweredMechanicalPressRecipe.Type.INSTANCE).stream().map(holder -> holder.value()).toList();
        List<LightningBatteryRecipe> lightningBatteryRecipes = recipeManager.getAllRecipesFor(LightningBatteryRecipe.Type.INSTANCE).stream().map(holder -> holder.value()).toList();
        ...
        registration.addRecipes(LightningBatteryCategory.LIGHTNING_BATTERY_TYPE, lightningBatteryRecipes);
