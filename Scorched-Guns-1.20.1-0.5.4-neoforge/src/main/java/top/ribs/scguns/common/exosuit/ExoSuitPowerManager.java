...
public class ExoSuitPowerManager {
    ...
    public static boolean consumeEnergy(Player player, String upgradeType, int energyRequired) {
        ...
        CompoundTag powerCoreTag = ItemStackNbtHelper.getOrCreateTag(powerCore);
    ...
    private static void updatePowerCoreInChestplate(ItemStack chestplate, ItemStack updatedPowerCore) {
        for (int slot = 0; slot < 4; slot++) {
            ItemStack upgradeItem = ExoSuitData.getUpgradeInSlot(chestplate, slot, ItemStackNbtHelper.EMPTY_REGISTRIES);
                    ...
                    if (upgradeData.contains("Upgrades")) {
                        ...
                        for (int i = 0; i < upgradeList.size(); i++) {
                            ...
                            if (slotTag.getInt("Slot") == slot) {
                                slotTag.put("Item", updatedPowerCore.save(ItemStackNbtHelper.EMPTY_REGISTRIES, new CompoundTag()));
                                break;
                            }
                        }
    ...
    public static boolean canUpgradeFunction(Player player, String upgradeType) {
        ...
        ItemStack upgradeItem = findUpgradeByType(player, armorPiece, upgradeType);
        if (upgradeItem.isEmpty()) {
            return false;
        }
        ...
        if (upgradeItem.getItem() instanceof EnergyUpgradeItem energyUpgrade) {
            ...
            ItemStack powerCore = findPowerCore(player, chestplate);
            if (powerCore.isEmpty()) {
                return false;
            }
            CompoundTag powerCoreTag = ItemStackNbtHelper.getTag(powerCore);
    ...
    private static ItemStack findPowerCore(Player player, ItemStack chestplate) {
        for (int slot = 0; slot < 4; slot++) {
            ItemStack upgradeItem = ExoSuitData.getUpgradeInSlot(chestplate, slot, player.level().registryAccess());
            if (!upgradeItem.isEmpty()) {
                ExoSuitUpgrade upgrade = ExoSuitUpgradeManager.getUpgradeForItem(upgradeItem);
                if (upgrade != null && upgrade.getType().equals("power_core")) {
                    return upgradeItem;
    ...
    private static ItemStack findUpgradeByType(Player player, ItemStack armorPiece, String upgradeType) {
        for (int slot = 0; slot < 4; slot++) {
            ItemStack upgradeItem = ExoSuitData.getUpgradeInSlot(armorPiece, slot, player.level().registryAccess());
            if (!upgradeItem.isEmpty()) {
                ExoSuitUpgrade upgrade = ExoSuitUpgradeManager.getUpgradeForItem(upgradeItem);
