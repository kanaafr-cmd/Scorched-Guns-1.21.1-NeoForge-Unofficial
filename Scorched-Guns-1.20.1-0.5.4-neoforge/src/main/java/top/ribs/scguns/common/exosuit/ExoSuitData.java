...
public class ExoSuitData {
    ...
    public static CompoundTag getUpgradeData(ItemStack exosuitPiece) {
        ...
        return ItemStackNbtHelper.getOrCreateTag(exosuitPiece).getCompound(UPGRADES_TAG);
    }
    ...
    public static void setUpgradeData(ItemStack exosuitPiece, CompoundTag upgradeData) {
        if (!exosuitPiece.isEmpty()) {
            ItemStackNbtHelper.getOrCreateTag(exosuitPiece).put(UPGRADES_TAG, upgradeData);
        }
    }
     ...
     */
    public static ItemStack getUpgradeInSlot(ItemStack exosuitPiece, int slot, net.minecraft.core.HolderLookup.Provider registryAccess) {
            ...
            for (int i = 0; i < upgradeList.size(); i++) {
                ...
                if (slotTag.getInt("Slot") == slot) {
                    if (slotTag.contains("Item")) {
                        return ItemStackNbtHelper.parse(slotTag.getCompound("Item"), registryAccess);
                    }
                }
            }
        }
    ...
    public static ItemStack getUpgradeInSlot(ItemStack exosuitPiece, int slot) {
        return getUpgradeInSlot(exosuitPiece, slot, ItemStackNbtHelper.EMPTY_REGISTRIES);
    }
