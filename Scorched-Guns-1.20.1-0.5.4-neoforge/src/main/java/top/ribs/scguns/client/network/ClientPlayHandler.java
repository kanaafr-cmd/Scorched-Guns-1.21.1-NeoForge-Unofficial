...
public class ClientPlayHandler {
    ...
    public static void handleEntityCasingEject(S2CMessageEntityCasingEject message) {
        ...
        ParticleType<?> particleType = BuiltInRegistries.PARTICLE_TYPE.get(message.getParticleLocation());
    ...
    public static void handleSyncReloadAnimation(int entityId, String animationName) {
        Minecraft mc = Minecraft.getInstance();
        ...
        if (!(heldItem.getItem() instanceof AnimatedGunItem animatedGun)) return;

        long id = GeoItem.getId(heldItem);
        AnimationController<GeoAnimatable> animationController = animatedGun.getAnimatableInstanceCache()
                .getManagerForId(id)
                .getAnimationControllers()
                .get("controller");

        if (animationController != null) {
            animationController.tryTriggerAnimation(animationName);
        }
    }
