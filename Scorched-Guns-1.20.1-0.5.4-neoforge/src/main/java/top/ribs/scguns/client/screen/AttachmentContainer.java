...
public class AttachmentContainer extends AbstractContainerMenu
{
    ...
    public void slotsChanged(Container inventoryIn)
    {
        ...
        for(int i = 0; i < this.getWeaponInventory().getContainerSize(); i++)
        {
            ...
            {
                attachments.put(("Barrel"), attachment.save(ItemStackNbtHelper.EMPTY_REGISTRIES, new CompoundTag()));
            }
            if(attachment.getItem() instanceof IAttachment)
            {
                attachments.put(((IAttachment) attachment.getItem()).getType().getTagKey(), attachment.save(ItemStackNbtHelper.EMPTY_REGISTRIES, new CompoundTag()));
            }
        }

        CompoundTag tag = ItemStackNbtHelper.getOrCreateTag(this.weapon);
