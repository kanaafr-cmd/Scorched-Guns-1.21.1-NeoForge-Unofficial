...
public class GunRenderingHandler {
    ...
    private void renderCooldownIndicator(Minecraft mc, Player player, ItemStack heldItem) {
        if (Config.CLIENT.display.cooldownIndicator.get()) {
            ...
            if (!gun.getGeneral().isAuto()) {
                float coolDown = player.getCooldowns().getCooldownPercent(heldItem.getItem(), mc.getTimer().getGameTimeDeltaPartialTick(true));
                if (coolDown > 0.0F) {
                    ...
                    int progress = Mth.clamp((int) Math.ceil(coolDown * barWidth), 0, barWidth);
                    guiGraphics.fill(x - 1, y - 1, x + barWidth + 1, y + barHeight + 1, 0xAA000000);
                    guiGraphics.fill(x, y, x + barWidth, y + barHeight, 0x66000000);
                    guiGraphics.fill(x, y, x + progress, y + barHeight, 0xFFFFFFFF);
                }
            }
        }
    }
    ...
    private void updateMelee() {
        ...
        Minecraft mc = Minecraft.getInstance();
        if (mc.player != null) {
            float currentTick = mc.player.tickCount + mc.getTimer().getGameTimeDeltaPartialTick(true);
    ...
    public void onRenderOverlay(RenderHandEvent event) {
        ...
        if (ItemStackNbtHelper.getTag(heldItem) != null) {
            if (ItemStackNbtHelper.getTag(heldItem).contains("Model", Tag.TAG_COMPOUND)) {
                overrideModel = ItemStackNbtHelper.parse(ItemStackNbtHelper.getTag(heldItem).getCompound("Model"));
            }
        }
    ...
    public void applyWeaponScale(ItemStack heldItem, PoseStack stack) {
        if (ItemStackNbtHelper.getTag(heldItem) != null) {
            CompoundTag compound = ItemStackNbtHelper.getTag(heldItem);
    ...
    private float getEquipProgress(float partialTicks) {
        if (this.equippedProgressMainHandField == null) {
            this.equippedProgressMainHandField = findItemInHandRendererField("mainHandHeight", "f_109302_");
        }
        if (this.prevEquippedProgressMainHandField == null) {
            this.prevEquippedProgressMainHandField = findItemInHandRendererField("oMainHandHeight", "f_109303_");
        }
    ...
    private static Field findItemInHandRendererField(String... fieldNames) {
        for (String fieldName : fieldNames) {
            try {
                Field field = ItemInHandRenderer.class.getDeclaredField(fieldName);
                field.setAccessible(true);
                return field;
            } catch (NoSuchFieldException ignored) {
            }
        }
        throw new IllegalStateException("Could not find ItemInHandRenderer equip progress field");
    }
