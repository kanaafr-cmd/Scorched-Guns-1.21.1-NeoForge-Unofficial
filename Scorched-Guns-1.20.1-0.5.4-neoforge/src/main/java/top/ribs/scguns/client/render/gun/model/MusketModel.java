 ...
 */
public class MusketModel implements IOverrideModel {
    ...
    public void render(float partialTicks, ItemDisplayContext transformType, ItemStack stack, ItemStack parent, LivingEntity entity, PoseStack matrixStack, MultiBufferSource buffer, int light, int overlay) {
        ...
        RenderUtil.renderModel(SpecialModels.MUSKET_MAIN.getModel(), stack, matrixStack, buffer, light, overlay);
        ...
        if (Gun.hasAttachmentEquipped(stack, IAttachment.Type.UNDER_BARREL)) {
            if (Gun.getAttachment(IAttachment.Type.UNDER_BARREL, stack).getItem() == ModItems.VERTICAL_GRIP.get())
                RenderUtil.renderModel(SpecialModels.MUSKET_VERTICAL_GRIP.getModel(), stack, matrixStack, buffer, light, overlay);
                ...
                RenderUtil.renderModel(SpecialModels.MUSKET_NETHERITE_BAYONET.getModel(), stack, matrixStack, buffer, light, overlay);
        ...
        if (entity.equals(Minecraft.getInstance().player)) {
            ...
            matrixStack.translate(0, 0.5, -0.5);
            RenderUtil.renderModel(SpecialModels.MUSKET_HAMMER.getModel(), stack, matrixStack, buffer, light, overlay);
            matrixStack.popPose();
            if (cooldown >= 0.9f) {
