...
public class AttachmentRenderer extends GeoRenderLayer<AnimatedGunItem> {
    ...
    public void updateAttachment(ItemStack attachmentStack) {
        ...
        model_resource = ResourceLocation.fromNamespaceAndPath("scguns", "geo/item/attachment/" + attachmentPath + ".geo.json");
        texture_resource = ResourceLocation.fromNamespaceAndPath("scguns", "textures/animated/attachment/" + attachmentPath + ".png");
        ...
        if (model_resource != null && texture_resource != null && itemStack != null) {
          AnimatedGunModel focusModel = new AnimatedGunModel(model_resource);
            ...
            if (bone.getName().matches("attachment_bone") && !bone.getName().matches(itemStack.getItem().toString()) && itemStack != null) {
                this.getRenderer().reRender(focusModel.getBakedModel(focusModelModelResource), poseStack, bufferSource, animatable, focusModelRenderLayer, bufferSource.getBuffer(focusModelRenderLayer), partialTick, packedLight, OverlayTexture.NO_OVERLAY, 0xFFFFFFFF);
            }
        }
