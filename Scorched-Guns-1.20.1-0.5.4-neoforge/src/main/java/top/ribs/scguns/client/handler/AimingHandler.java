...
{
    private static final boolean DEBUG_ADS_RELOAD = Boolean.getBoolean("scguns.debug.adsreload");
    ...
    private String lastAdsBlockReason = "";
    ...
    private static void debugLog(String message)
    {
        if(DEBUG_ADS_RELOAD)
        {
            ScorchedGuns.LOGGER.info("[ADS/Reload Debug] {}", message);
        }
    }
    ...
    public void onClientTick(TickEvent.ClientTickEvent event)
    {
        ...
        if(isReloading || inCriticalPhase) {
            debugLog("Aiming blocked by reload state. isReloading=" + isReloading + ", inCriticalPhase=" + inCriticalPhase);
            this.aiming = false;
            wasKeyPressed = KeyBinds.getAimMapping().isDown();
            if(ModSyncedDataKeys.AIMING.getValue(player)) {
                ModSyncedDataKeys.AIMING.setValue(player, false);
        ...
        if(shouldBeAiming) {
            if(heldItem.getItem() instanceof GunItem) {
                ...
                if(!gun.canAimDownSight()) {
                    ...
                    this.aiming = false;
                    if (!"no_ads_module".equals(this.lastAdsBlockReason)) {
                        debugLog("ADS blocked: gun has no ADS module/scope support.");
                        this.lastAdsBlockReason = "no_ads_module";
                    }
                }
                else if(gripType == GripType.ONE_HANDED && !player.getOffhandItem().isEmpty()) {
                    shouldBeAiming = false;
                    this.aiming = false;
                    if (!"offhand_block".equals(this.lastAdsBlockReason)) {
                        debugLog("ADS blocked: one-handed gun with non-empty offhand.");
                        this.lastAdsBlockReason = "offhand_block";
                    }
                }
            }
            else {
                shouldBeAiming = false;
                this.aiming = false;
                if (!"not_gun".equals(this.lastAdsBlockReason)) {
                    debugLog("ADS blocked: held item is not a gun.");
                    this.lastAdsBlockReason = "not_gun";
                }
            }
        }
        ...
        if(shouldBeAiming)
        {
            this.lastAdsBlockReason = "";
            if(!ModSyncedDataKeys.AIMING.getValue(player))
            {
                debugLog("Sync AIMING -> true");
