...
public class SniperTurretMenu extends AbstractContainerMenu {
    private final Level level;
    private final BlockPos pos;

    public SniperTurretMenu(int id, Inventory playerInventory, FriendlyByteBuf extraData) {
        this(id, playerInventory, playerInventory.player.level().getBlockEntity(extraData.readBlockPos()));
    }

    public SniperTurretMenu(int id, Inventory playerInventory, BlockEntity entity) {
        super(ModMenuTypes.SNIPER_TURRET_MENU.get(), id);

        if (!(entity instanceof SniperTurretBlockEntity turretBlockEntity)) {
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level, pos), player, ModBlocks.SNIPER_TURRET.get());
    }
    ...
    public SniperTurretBlockEntity getBlockEntity() {
        BlockEntity blockEntity = this.level.getBlockEntity(this.pos);
        return blockEntity instanceof SniperTurretBlockEntity turretBlockEntity ? turretBlockEntity : null;
    }
}
