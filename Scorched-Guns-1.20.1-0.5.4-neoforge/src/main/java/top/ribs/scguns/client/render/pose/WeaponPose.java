...
public abstract class WeaponPose implements IHeldAnimation
{
    ...
    /**
     * Gets the pose of the player when melee attacking
     */
    protected abstract AimPose getMeleePose();
    /**
     * Gets the pose of the player when banzai attacking
     */
    protected abstract AimPose getBanzaiPose();
    ...
    protected float getPlayerPitch(Player player)
    {
        ...
        return Mth.lerp(Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(true), player.xRotO, player.getXRot()) / 90F;
    }
