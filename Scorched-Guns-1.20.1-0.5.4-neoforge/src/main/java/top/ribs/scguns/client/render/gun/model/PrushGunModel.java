    ...
    private void renderBarrelAttachments(PoseStack matrixStack, MultiBufferSource buffer, ItemStack stack, int light, int overlay) {
        ...
        if (Gun.hasAttachmentEquipped(stack, IAttachment.Type.BARREL)) {
            if (Gun.getAttachment(IAttachment.Type.BARREL, stack).getItem() == ModItems.EXTENDED_BARREL.get()) {
                RenderUtil.renderModel(SpecialModels.PRUSH_GUN_EXT_BARREL.getModel(), stack, matrixStack, buffer, light, overlay);
        ...
        if (!hasExtendedBarrel) {
            RenderUtil.renderModel(SpecialModels.PRUSH_GUN_STAN_BARREL.getModel(), stack, matrixStack, buffer, light, overlay);
        }
    }
