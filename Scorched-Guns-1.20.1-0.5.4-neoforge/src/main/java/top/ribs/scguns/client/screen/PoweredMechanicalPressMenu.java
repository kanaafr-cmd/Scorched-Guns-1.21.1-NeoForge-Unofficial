...
import top.ribs.scguns.compat.net.neoforged.neoforge.common.ForgeHooks;
...
public class PoweredMechanicalPressMenu extends AbstractContainerMenu {
    public final PoweredMechanicalPressBlockEntity blockEntity;
    ...
    public PoweredMechanicalPressMenu(int id, Inventory inv, FriendlyByteBuf extraData) {
        this(id, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(4));
    }

    public PoweredMechanicalPressMenu(int id, Inventory inv, BlockEntity entity, ContainerData data) {
        super(ModMenuTypes.POWERED_MECHANICAL_PRESS_MENU.get(), id);
        checkContainerSize(inv, 5);
        blockEntity = ((PoweredMechanicalPressBlockEntity) entity);
        ...
        this.blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER, null).ifPresent(handler -> {
            ...
            this.addSlot(new SlotItemHandler(handler, 2, 62, yLevel)); // Input 3
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()), player, ModBlocks.POWERED_MECHANICAL_PRESS.get());
    }
    ...
    private boolean isMoldItem(ItemStack stack) {
        ...
        return recipeManager.getAllRecipesFor(PoweredMechanicalPressRecipe.Type.INSTANCE).stream()
                .map(holder -> holder.value()).anyMatch(recipe -> recipe.getMoldItem().test(stack));
    }

    private boolean isInputItem(ItemStack stack) {
        if (level == null) return false;
        RecipeManager recipeManager = level.getRecipeManager();
        return recipeManager.getAllRecipesFor(PoweredMechanicalPressRecipe.Type.INSTANCE).stream().map(holder -> holder.value())
