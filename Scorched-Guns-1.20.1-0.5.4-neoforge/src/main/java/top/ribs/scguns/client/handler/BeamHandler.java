...
public class BeamHandler {
    ...
    public static float[] getBeamColorForWeapon(ItemStack heldItem, Gun modifiedGun) {
        ...
        float time = (Minecraft.getInstance().level.getGameTime() + Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(true)) % 100;
    ...
    public static void onRenderLevel(RenderLevelStageEvent event) {
        ...
        setupRenderContext(poseStack, event.getPartialTick().getGameTimeDeltaPartialTick(true), player);

        renderActiveBeams(mc, event.getPartialTick().getGameTimeDeltaPartialTick(true), poseStack, bufferSource);
