...
public class ShotgunTurretMenu extends AbstractContainerMenu {
    ...
    public ShotgunTurretMenu(int id, Inventory playerInventory, FriendlyByteBuf extraData) {
        this(id, playerInventory, playerInventory.player.level().getBlockEntity(extraData.readBlockPos()));
    }

    public ShotgunTurretMenu(int id, Inventory playerInventory, BlockEntity entity) {
        super(ModMenuTypes.SHOTGUN_TURRET_MENU.get(), id);

        if (!(entity instanceof ShotgunTurretBlockEntity turretBlockEntity)) {
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level, pos), player, ModBlocks.SHOTGUN_TURRET.get());
    }
    ...
    public ShotgunTurretBlockEntity getBlockEntity() {
        BlockEntity blockEntity = this.level.getBlockEntity(this.pos);
        return blockEntity instanceof ShotgunTurretBlockEntity turretBlockEntity ? turretBlockEntity : null;
    }
}
