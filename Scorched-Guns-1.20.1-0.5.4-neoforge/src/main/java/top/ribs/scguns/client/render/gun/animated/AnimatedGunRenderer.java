...
public class AnimatedGunRenderer extends GeoItemRenderer<AnimatedGunItem> implements GeoRenderer<AnimatedGunItem> {
    ...
    private static final Map<UUID, Map<String, Vec3>> PLAYER_BONE_ROTATIONS = new HashMap<>();
    ...
    public void renderByItem(ItemStack stack, ItemDisplayContext transformType,
        ...
        if (ItemStackNbtHelper.getTag(stack) != null && ItemStackNbtHelper.getTag(stack).contains("Model", 10)) {
            overrideModel = ItemStackNbtHelper.parse(ItemStackNbtHelper.getTag(stack).getCompound("Model"));
        }
        ...
        if (stack.getItem() instanceof AnimatedGunItem && transformType == ItemDisplayContext.FIRST_PERSON_RIGHT_HAND) {
            ...
            this.applyBobbingTransforms(poseStack, client.getTimer().getGameTimeDeltaPartialTick(true));
            ...
            this.applySwayTransforms(poseStack, modifiedGun, stack, localPlayer, translateX, translateY, translateZ, client.getTimer().getGameTimeDeltaPartialTick(true));

            if (ShootingHandler.get().isShooting() && !GunModifierHelper.isSilencedFire(stack)) {
                this.renderMuzzleFlash(client.player, poseStack, bufferSource, stack, ItemDisplayContext.FIRST_PERSON_RIGHT_HAND, client.getTimer().getGameTimeDeltaPartialTick(true));
            }
        }
        ...
        if (transformType == ItemDisplayContext.GUI) {
            ...
            packedLight = LightTexture.pack(blockLight,
                    player.level().getBrightness(LightLayer.SKY, BlockPos.containing(player.getEyePosition(client.getTimer().getGameTimeDeltaPartialTick(true)))));
        }
    ...
    private int calculateBlockLight(Player player, ItemStack stack) {
        int blockLight = player.isOnFire() ? 15 :
                player.level().getBrightness(LightLayer.BLOCK,
                        BlockPos.containing(player.getEyePosition(Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(true))));
    ...
    public static Vec3 getBoneRotation(UUID playerId, String boneName) {
        Map<String, Vec3> rotations = PLAYER_BONE_ROTATIONS.get(playerId);
        return rotations != null ? rotations.get(boneName) : null;
    }
    ...
    public void renderRecursively(PoseStack poseStack, AnimatedGunItem animatable, GeoBone bone,
        ...
        if (this.currentRenderStack != null && client.level != null) {
            // Find the player holding this stack (might be the local player or another player)
            for (Player player : client.level.players()) {
                if (player.getMainHandItem() == this.currentRenderStack || player.getOffhandItem() == this.currentRenderStack) {
                    Map<String, Vec3> rotations = PLAYER_BONE_ROTATIONS.computeIfAbsent(player.getUUID(), k -> new HashMap<>());
                    rotations.put(bone.getName(), new Vec3(bone.getRotX(), bone.getRotY(), bone.getRotZ()));
                    break;
                }
            }
        }
        ...
        if (boneType == BoneType.ARM) {
            if (this.currentDisplayContext != ItemDisplayContext.GUI) {
                shouldRenderCustomArms = shouldRenderArms();
                bone.setHidden(true);
        ...
        if (shouldRenderCustomArms && client.player != null) {
            renderPlayerArms(client, poseStack, bufferSource, bone, animatable, packedLight, packedOverlay);
        }

        if (boneType == BoneType.ATTACHMENT) {
            renderAttachments(bone, this.currentRenderStack, poseStack, renderType, buffer,
                    packedLight, client.getTimer().getGameTimeDeltaPartialTick(true), packedOverlay);
        }
        ...
        super.renderRecursively(poseStack, animatable, bone, renderType, bufferSource,
                buffer, isReRender, partialTick, packedLight, packedOverlay, renderColor);
    ...
    private void renderPlayerArms(Minecraft client, PoseStack poseStack, MultiBufferSource bufferSource, GeoBone bone,
        ...
        if (isRightArm(bone)) {
            ...
            renderRightArm(poseStack, playerEntityModel, arm, bone, packedLight, packedOverlay);
            VertexConsumer sleeve = bufferSource.getBuffer(RenderType.entityTranslucent(playerSkin));
            renderRightSleeve(poseStack, playerEntityModel, sleeve, bone, packedLight, packedOverlay);
        } else {
            renderLeftArm(poseStack, playerEntityModel, bufferSource, playerSkin, bone, packedLight, packedOverlay,
    ...
    private void renderRightArm(PoseStack poseStack, PlayerModel<AbstractClientPlayer> playerEntityModel,
                                VertexConsumer arm, GeoBone bone,
                                int packedLight, int packedOverlay) {
        poseStack.pushPose();
    ...
    private void renderRightSleeve(PoseStack poseStack, PlayerModel<AbstractClientPlayer> playerEntityModel,
                                   VertexConsumer sleeve, GeoBone bone,
                                   int packedLight, int packedOverlay) {
        poseStack.pushPose();
        poseStack.scale(0.66F, 0.78F, 0.66F);
        poseStack.translate(0.25, -0.1, 0.1625);
    ...
    private void renderLeftArm(PoseStack poseStack, PlayerModel<AbstractClientPlayer> playerEntityModel,
                               MultiBufferSource bufferSource, ResourceLocation playerSkin, GeoBone bone,
    ...
    private void renderAttachments(GeoBone bone, ItemStack stack, PoseStack poseStack, RenderType renderType,
        ...
        Gun modifiedGun = ((GunItem) stack.getItem()).getModifiedGun(stack);
        CompoundTag gunTag = ItemStackNbtHelper.getOrCreateTag(stack);
