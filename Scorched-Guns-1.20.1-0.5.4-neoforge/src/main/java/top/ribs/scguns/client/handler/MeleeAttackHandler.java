...
public class MeleeAttackHandler {
    ...
    public static void setMeleeCooldown(Player player, ItemStack heldItem, GunItem gunItem) {
        ...
        ItemStackNbtHelper.setTag(heldItem, tag);
    }
    ...
    private static void applySpecialEnchantmentsFromBayonet(ItemStack gunStack, LivingEntity target, Player player, GunItem gunItem) {
        for (IAttachment.Type type : IAttachment.Type.values()) {
            ...
            if (attachmentStack.getItem() instanceof BayonetItem) {
                for (var entry : EnchantmentHelper.getEnchantmentsForCrafting(attachmentStack).entrySet()) {
                    Holder<Enchantment> enchantment = entry.getKey();
                    int level = entry.getIntValue();
                    applyEnchantmentEffects(enchantment, level, target, player);
                    if (enchantment.is(ModEnchantments.CORRODED)) {
            ...
            if (attachmentStack.getItem() instanceof BayonetItem) {
                for (var entry : EnchantmentHelper.getEnchantmentsForCrafting(attachmentStack).entrySet()) {
                    ...
                    if (enchantment.is(Enchantments.SHARPNESS)) {
                        enchantmentDamage += (1.0F + Math.max(0, level - 1) * 0.5F) * ENCHANTMENT_DAMAGE_SCALING_FACTOR;
                    } else if (enchantment.is(Enchantments.SMITE) && target.getType().is(EntityTypeTags.SENSITIVE_TO_SMITE)) {
                        enchantmentDamage += level * 2.5F * ENCHANTMENT_DAMAGE_SCALING_FACTOR;
                    } else if (enchantment.is(Enchantments.BANE_OF_ARTHROPODS) && target.getType().is(EntityTypeTags.SENSITIVE_TO_BANE_OF_ARTHROPODS)) {
                        enchantmentDamage += level * 2.5F * ENCHANTMENT_DAMAGE_SCALING_FACTOR;
                    } else if (enchantment.is(ModEnchantments.CORRODED) && isBotEntity(target)) {
    ...
    private static void applyEnchantmentEffects(Holder<Enchantment> enchantment, int level, LivingEntity target, Player player) {
        ...
        if (enchantment.is(Enchantments.FIRE_ASPECT)) {
            ...
            spawnParticlePacket(player, target, ParticleTypes.FLAME, 6);
        ...
        } else if (enchantment.is(Enchantments.SHARPNESS)) {
    ...
    private static void spawnParticlePacket(Player player, LivingEntity target, ParticleOptions particle, int count) {
        if (player instanceof ServerPlayer serverPlayer) {
            ClientboundLevelParticlesPacket particlePacket = new ClientboundLevelParticlesPacket(
                    particle,
                    true,
    ...
    private static void applyKnockback(Player player, LivingEntity target, ItemStack stack) {
        int knockbackLevel = getAttachmentEnchantmentLevel(stack, Enchantments.KNOCKBACK);
        Vec3 direction = target.position().subtract(player.position()).normalize();
        target.knockback(0.4F + (knockbackLevel * 0.5F), -direction.x(), -direction.z());
    }

    private static int getAttachmentEnchantmentLevel(ItemStack gunStack, net.minecraft.resources.ResourceKey<Enchantment> enchantment) {
        ...
        for (IAttachment.Type type : IAttachment.Type.values()) {
            ItemStack attachmentStack = gunItem.getAttachment(gunStack, type);
            if (attachmentStack.getItem() instanceof BayonetItem) {
                highestLevel = Math.max(highestLevel, EnchantmentHelper.getEnchantmentsForCrafting(attachmentStack).entrySet().stream()
