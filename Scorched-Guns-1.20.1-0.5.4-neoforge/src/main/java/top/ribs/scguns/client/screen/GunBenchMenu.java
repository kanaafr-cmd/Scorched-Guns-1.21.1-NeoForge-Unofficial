    ...
    public void slotsChanged(Container container) {
        containerAccess.execute((level, pos) -> {
            if (!level.isClientSide) {
                ...
                Optional<GunBenchRecipe> recipe = level.getRecipeManager().getRecipeFor(GunBenchRecipe.Type.INSTANCE, ScGunsRecipeInput.of(craftingContainer), level).map(holder -> holder.value());

                if (recipe.isPresent()) {
                    ItemStack result = recipe.get().assemble(ScGunsRecipeInput.of(craftingContainer), level.registryAccess());
    ...
    private void consumeIngredients() {
        containerAccess.execute((level, pos) -> {
            if (!level.isClientSide) {
                ...
                Optional<GunBenchRecipe> recipeOptional = level.getRecipeManager().getRecipeFor(GunBenchRecipe.Type.INSTANCE, ScGunsRecipeInput.of(craftingContainer), level).map(holder -> holder.value());
