...
public class RaidConfig {
    ...
    private static BossData parseBossData(JsonObject json) {
        try {
            ...
            if (json.has("weapon")) {
                ...
                Item weaponItem = BuiltInRegistries.ITEM.get(ResourceLocation.parse(weaponObj.get("item").getAsString()));
            ...
            if (json.has("armor")) {
                ...
                for (JsonElement element : armorArray) {
                    ...
                    Item armorItem = BuiltInRegistries.ITEM.get(ResourceLocation.parse(armorObj.get("item").getAsString()));
            ...
            ResourceLocation lootTable = json.has("special_loot_table") ?
                    ResourceLocation.parse(json.get("special_loot_table").getAsString()) : null;
    ...
    private static MountData parseMountData(JsonObject json) {
        try {
            ...
            EntityType<?> mountType = BuiltInRegistries.ENTITY_TYPE.get(ResourceLocation.parse(mountId));
            if (mountType == null) {
                ...
                for (JsonElement element : typesArray) {
                    ...
                    if (typeObj.has("weapons")) {
                        ...
                        for (JsonElement weaponElement : weaponsArray) {
                            Item weapon = BuiltInRegistries.ITEM.get(ResourceLocation.parse(weaponElement.getAsString()));
                            if (weapon != null) {
                                weapons.add(weapon);
                            }
                        }
