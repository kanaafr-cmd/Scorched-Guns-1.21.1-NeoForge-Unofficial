 ...
 * Author: MrCrayfish
 */
public class NailBombItem extends AmmoItem
{
    protected int maxCookTime;

    public NailBombItem(Properties properties, int maxCookTime)
    {
        super(properties);
    ...
    public void onUseTick(Level level, LivingEntity player, ItemStack stack, int count)
    {
        ...
        if(duration == 10)
            player.level().playLocalSound(player.getX(), player.getY(), player.getZ(), SoundEvents.TNT_PRIMED, SoundSource.PLAYERS, 1.0F, 1.0F, false);
    }
    ...
    {
        return new ThrowableNailBombEntity(world, entity, timeLeft);
    }
