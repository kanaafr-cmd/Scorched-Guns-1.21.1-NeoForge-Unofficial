...
public class ExoSuitItem extends ArmorItem implements GeoItem {
    ...
    private void openExoSuitScreen(ServerPlayer player, InteractionHand hand) {
        player.openMenu(new ExoSuitMenuProvider(hand), buf -> {
            buf.writeEnum(hand);
        });
    }
    ...
    public void appendHoverText(ItemStack stack, net.minecraft.world.item.Item.TooltipContext context, List<Component> tooltip, TooltipFlag flag) {
        ...
        if (ExoSuitData.hasUpgrades(stack)) {
            ...
            addPowerCoreInfo(stack, context, tooltip);

            addPouchInfo(stack, context, tooltip);

            addUpgradeDurabilityInfo(stack, context, tooltip);
        } else {
    ...
    private void addPowerCoreInfo(ItemStack stack, net.minecraft.world.item.Item.TooltipContext context, List<Component> tooltip) {
        if (this.getType() != ArmorItem.Type.CHESTPLATE) {
            return;
        }

        ItemStack powerCore = findPowerCore(stack, context.registries());
        if (!powerCore.isEmpty()) {
            IEnergyStorage energyStorage = powerCore.getCapability(Capabilities.EnergyStorage.ITEM);
            int energyStored = energyStorage != null ? energyStorage.getEnergyStored() : 0;
            int maxEnergy = energyStorage != null ? energyStorage.getMaxEnergyStored() : 0;
    ...
    private ItemStack findPowerCore(ItemStack stack, net.minecraft.core.HolderLookup.Provider registries) {
        for (int slot = 0; slot < getMaxUpgradeSlots(); slot++) {
            ItemStack upgradeItem = ExoSuitData.getUpgradeInSlot(stack, slot, registries);
    ...
    private void addUpgradeDurabilityInfo(ItemStack stack, net.minecraft.world.item.Item.TooltipContext context, List<Component> tooltip) {
        boolean hasAnyDamageableUpgrades = false;

        for (int slot = 0; slot < getMaxUpgradeSlots(); slot++) {
            ItemStack upgradeItem = ExoSuitData.getUpgradeInSlot(stack, slot, context.registries());
            if (!upgradeItem.isEmpty()) {
                ExoSuitUpgrade upgrade = ExoSuitUpgradeManager.getUpgradeForItem(upgradeItem);
    ...
    private void addPouchInfo(ItemStack stack, net.minecraft.world.item.Item.TooltipContext context, List<Component> tooltip) {
        if (this.getType() != ArmorItem.Type.CHESTPLATE) {
            return;
        }

        ItemStack pouchUpgrade = findPouchUpgrade(stack, context.registries());
        if (!pouchUpgrade.isEmpty()) {
            ...
            if (upgrade != null) {
                ...
                if (isPouchEmpty(stack, pouchUpgrade, context.registries())) {
    ...
    private ItemStack findPouchUpgrade(ItemStack stack, net.minecraft.core.HolderLookup.Provider registries) {
        for (int slot = 0; slot < getMaxUpgradeSlots(); slot++) {
            ItemStack upgradeItem = ExoSuitData.getUpgradeInSlot(stack, slot, registries);
            if (!upgradeItem.isEmpty()) {
                ExoSuitUpgrade upgrade = ExoSuitUpgradeManager.getUpgradeForItem(upgradeItem);
    ...
    private boolean isPouchEmpty(ItemStack chestplate, ItemStack pouchUpgrade, net.minecraft.core.HolderLookup.Provider registries) {
        try {
            ...
            net.neoforged.neoforge.items.ItemStackHandler handler = new net.neoforged.neoforge.items.ItemStackHandler(upgrade.getDisplay().getStorageSize());
