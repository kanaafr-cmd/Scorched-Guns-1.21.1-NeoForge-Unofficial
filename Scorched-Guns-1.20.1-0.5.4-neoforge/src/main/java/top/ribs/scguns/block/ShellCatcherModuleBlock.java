...
public class ShellCatcherModuleBlock extends BaseTurretModuleBlock {
    ...
    public static final MapCodec<ShellCatcherModuleBlock> CODEC = simpleCodec(ShellCatcherModuleBlock::new);
    ...
    protected ItemInteractionResult useItemOn(ItemStack heldStack, BlockState state, Level level, BlockPos pos, Player player, InteractionHand hand, BlockHitResult hit) {
        if (!level.isClientSide) {
            ...
            if (blockEntity instanceof ShellCatcherModuleBlockEntity) {
                ((ServerPlayer) player).openMenu((ShellCatcherModuleBlockEntity) blockEntity, buf -> buf.writeBlockPos(pos));
            }
        }
