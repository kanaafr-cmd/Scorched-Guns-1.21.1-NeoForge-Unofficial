...
public class TraumaHookRenderer extends EntityRenderer<TraumaHookEntity> {
    ...
    private void renderHook(TraumaHookEntity entity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
        ...
        vertex(vertexConsumer, pose, matrix4f, packedLight, 0.0F, 0, 0, 1);
        vertex(vertexConsumer, pose, matrix4f, packedLight, 1.0F, 0, 1, 1);
        vertex(vertexConsumer, pose, matrix4f, packedLight, 1.0F, 1, 1, 0);
        vertex(vertexConsumer, pose, matrix4f, packedLight, 0.0F, 1, 0, 0);
    ...
    private static void renderLineSegment(float deltaX, float deltaY, float deltaZ, VertexConsumer consumer, PoseStack.Pose pose, float start, float end) {
        ...
        consumer.addVertex(pose.pose(), x, y, z)
                .setColor(32, 32, 32, 255) // Dark gray line
                .setNormal(pose, nextX, nextY, nextZ)
                ;
    }

    private static void vertex(VertexConsumer consumer, PoseStack.Pose pose, Matrix4f matrix, int light, float x, int y, int u, int v) {
