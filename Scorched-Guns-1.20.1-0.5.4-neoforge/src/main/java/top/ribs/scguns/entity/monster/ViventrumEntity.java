...
public class ViventrumEntity extends TamableAnimal {
    ...
    protected void registerGoals() {
        ...
        if (!hasGun) {
            this.goalSelector.addGoal(2, new MeleeAttackGoal(this, 1.0D, false) {
                ...
                protected void checkAndPerformAttack(LivingEntity pEnemy, double pDistToEnemySqr) {
                    if (pDistToEnemySqr <= this.mob.getBbWidth() * 2.0F * this.mob.getBbWidth() * 2.0F + pEnemy.getBbWidth() && this.getTicksUntilNextAttack() <= 0 && !ViventrumEntity.this.isAttacking()) {
        ...
        if (this.isTame()) {
            this.goalSelector.addGoal(4, new ViventrumFollowOwnerGoal(this, 1.3, 10.0F, 2.0F));
        }
    ...
    public @NotNull InteractionResult mobInteract(@NotNull net.minecraft.world.entity.player.Player player, @NotNull InteractionHand hand) {
        ...
        if (this.level().isClientSide) {
            ...
            if (this.isTame()) {
                ...
                if (this.random.nextInt(3) == 0 && !top.ribs.scguns.compat.net.neoforged.neoforge.event.ForgeEventFactory.onAnimalTame(this, player)) {
                    this.tame(player);
    ...
    public boolean canReplaceCurrentItem(ItemStack candidate, ItemStack existing) {
        if (existing.isEmpty()) {
            return this.getEquipmentSlotForItem(candidate) == EquipmentSlot.HEAD;
        }
        return false;
    }
        ...
        if (defensive) {
            Objects.requireNonNull(this.getAttribute(Attributes.ARMOR)).addTransientModifier(
                    new net.minecraft.world.entity.ai.attributes.AttributeModifier(
                            ResourceLocation.fromNamespaceAndPath("scguns", "viventrum_defensive_bonus"), DEFENSIVE_ARMOR_BONUS,
                            net.minecraft.world.entity.ai.attributes.AttributeModifier.Operation.ADD_VALUE));
        } else {
            Objects.requireNonNull(this.getAttribute(Attributes.ARMOR)).removeModifier(
                    new net.minecraft.world.entity.ai.attributes.AttributeModifier(
                            ResourceLocation.fromNamespaceAndPath("scguns", "viventrum_defensive_bonus"), DEFENSIVE_ARMOR_BONUS,
                            net.minecraft.world.entity.ai.attributes.AttributeModifier.Operation.ADD_VALUE).id());
        }
    }
